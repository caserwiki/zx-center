...
`).appendTo($li);
        ...
        try {
            image.once('load', function () {
                ...
                if (! _this.validateDimensions(file)) {
                    Zx.error(lang.trans('dimensions') || 'The image dimensions is invalid.');
    ...
    validateDimensions(file) {
        let _this = this,
            ...
            height = file._info.height,
            isset = Zx.helpers.isset;

        // The image dimensions is invalid.
        if (! parent.isImage() || ! _this.isImage(file) || ! Zx.helpers.len(options.dimensions)) {
            return true;
        }
