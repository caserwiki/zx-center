...
let tpl = '<div class="zx-loading d-flex items-center align-items-center justify-content-center pin" style="{style}">{svg}</div>',
    loading = '.zx-loading',
    LOADING_SVG = [
...
class Loading {
    constructor(Zx, options) {
        options = $.extend({
            container: Zx.config.pjax_container_selector,
            zIndex: 100,
            width: '52px',
            color: Zx.color.dark60,
            background: '#fff',
            style: '',
            svg: LOADING_SVG[0]
        }, options);
...
function extend(Zx) {
    ...
    Zx.loading = function (options) {
        ...
        var win = $(window),
            // 容器
            $container = $('<div class="zx-loading" style="z-index:'+options.zIndex+';width:300px;position:fixed"></div>'),
            // 遮罩层直接沿用layer
            shadow = $('<div class="layui-layer-shade zx-loading" style="z-index:'+(options.zIndex-2)+'; background-color:'+options.shade+'"></div>');
    ...
    $.fn.buttonLoading = function (start) {
        ...
        loadingId = 'ld-'+Zx.helpers.random();
