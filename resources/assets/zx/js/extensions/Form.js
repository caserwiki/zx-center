...
class Form {
    ...
    init() {
        ...
        Zx.confirm(confirm.title, confirm.content, function () {
            _this.submit();
        });
    }
    ...
    showError($form, column, errors) {
        ...
        if (! $field) {
            if (Zx.helpers.len(errors) && errors.length) {
                Zx.error(errors.join("  \n  "));
            }
            return;
        }
    ...
    isValueChanged($field, column) {
        return ! Zx.helpers.equal(this.originalValues[column], this.getFieldValue($field));
    }
