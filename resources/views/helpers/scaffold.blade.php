@php
    $timestamps = Zx\Admin\Widgets\Checkbox::make('timestamps')->inline();
    $timestamps->options([1 => 'Created_at & Updated_at'])->check(1);

    $soft = Zx\Admin\Widgets\Checkbox::make('soft_deletes')->inline();
    $soft->options([1 => (trans('admin.scaffold.soft_delete'))]);
    if (old('soft_deletes') != NULL) {
        $soft->check(1);
    }

    $actionCreators = Zx\Admin\Widgets\Checkbox::make('create[]')->inline();
