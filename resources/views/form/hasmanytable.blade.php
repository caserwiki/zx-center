                ...
                @foreach($forms as $pk => $form)
                    <tr class="has-many-table-{{$columnClass}}-form fields-group">
                        ...
                        @foreach($form->fields() as $field)
                            ...
                            @if (is_a($field, Zx\Admin\Form\Field\Hidden::class))
...
(function () {
    ...
    function replaceNestedFormIndex(value) {
        return String(value).replace(/{{ $parentKey ?: Zx\Admin\Form\NestedForm::DEFAULT_KEY_NAME }}/g, nestedIndex);
    }
    ...
    $(document).off('click', container+' .remove').on('click', container+' .remove', function () {
        ...
        $form.find('.{{ Zx\Admin\Form\NestedForm::REMOVE_FLAG_CLASS }}').val(1);
    });
})();
</script>
