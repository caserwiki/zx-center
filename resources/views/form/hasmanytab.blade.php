...
<div class="nav-tabs-custom has-many-{{$columnClass}}">
    ...
    <template class="nav-tab-tpl">
        <li class="new nav-item">
            <a href="#{{ $relationName . '_new_' . \Zx\Admin\Form\NestedForm::DEFAULT_KEY_NAME }}" class="nav-link" data-toggle="tab">
                &nbsp;New {{ \Zx\Admin\Form\NestedForm::DEFAULT_KEY_NAME }} <i class="feather icon-alert-circle text-red d-none"></i>
            </a>
            <i class="close-tab feather icon-trash text-red" ></i>
        </li>
    </template>
    <template class="pane-tpl">
        <div class="tab-pane fields-group new" id="{{ $relationName . '_new_' . Zx\Admin\Form\NestedForm::DEFAULT_KEY_NAME }}">
            {!! $template !!}
        </div>
    </template>
...
<script>
    ...
    $(container+' > .nav').off('click', 'i.close-tab').on('click', 'i.close-tab', function(){
        ...
        if( $pane.hasClass('new') ){
            ...
            $pane.removeClass('active').find('.{{ Zx\Admin\Form\NestedForm::REMOVE_FLAG_CLASS }}').val(1);
        }
    ...
    function replaceNestedFormIndex(value) {
        return String(value).replace(/{{ Zx\Admin\Form\NestedForm::DEFAULT_KEY_NAME }}/g, nestedIndex);
    }
