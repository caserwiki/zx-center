...
use Closure;
use Zx\Admin\Contracts\ExceptionHandler;
use Zx\Admin\Contracts\Repository;
use Zx\Admin\Exception\InvalidArgumentException;
use Zx\Admin\Http\Controllers\AuthController;
use Zx\Admin\Http\JsonResponse;
use Zx\Admin\Layout\Menu;
...
use Zx\Admin\Traits\HasAssets;
use Zx\Admin\Traits\HasHtml;
...
class Admin
{
    ...
    {
        return sprintf('Zx Center <comment>version</comment> <info>%s</info>', static::VERSION);
    }
     ...
     * 配置.
     *
     * @return \Zx\Admin\Support\Setting
     */
     ...
     * 上下文管理.
     *
     * @return \Zx\Admin\Support\Context
     */
     ...
     * 翻译器.
     *
     * @return \Zx\Admin\Support\Translator
     */
     ...
     *
     * @param  string $name
     * @return \Zx\Admin\Extend\Manager|\Zx\Admin\Extend\ServiceProvider|null
     */
    ...
    public static function routes()
    {
        ...
        if (config('admin.auth.enable', true)) {
            app('router')->group($attributes, function ($router) {
                ...
                $router->namespace('Zx\Admin\Http\Controllers')->group(function ($router) {
                    /* @var \Illuminate\Routing\Router $router */
                ...
                $router->resource('auth/extensions', 'Zx\Admin\Http\Controllers\ExtensionController', ['only' => ['index', 'store', 'update']]);
    ...
    public static function registerHelperRoutes()
    {
        ...
        app('router')->group($attributes, function ($router) {
            /* @var \Illuminate\Routing\Router $router */
            $router->get('helpers/scaffold', 'Zx\Admin\Http\Controllers\ScaffoldController@index');
            $router->post('helpers/scaffold', 'Zx\Admin\Http\Controllers\ScaffoldController@store');
            $router->post('helpers/scaffold/table', 'Zx\Admin\Http\Controllers\ScaffoldController@table');
            $router->get('helpers/icons', 'Zx\Admin\Http\Controllers\IconController@index');
        });
    }
}
