...
use Zx\Admin\Form;
use Zx\Admin\Grid;
use Zx\Admin\Http\Actions\Extensions\InstallFromLocal;
use Zx\Admin\Http\Actions\Extensions\Marketplace;
use Zx\Admin\Http\Displayers\Extensions;
use Zx\Admin\Http\Repositories\Extension;
use Zx\Admin\Layout\Content;
use Zx\Admin\Support\Helper;
use Zx\Admin\Support\StringOutput;
        ...
        return new Grid(new Extension(), function (Grid $grid) {
            ...
            $grid->quickCreate(function (Grid\Tools\QuickCreate $create) {
                $create->text('name')
                    ...
                    ->placeholder('Input Name. Eg: zx-center/demo')
                    ->required();
                $create->text('namespace')
                    ->attribute('style', 'width:240px')
                    ->placeholder('Input Namespace. Eg: ZxCenter\\Demo');
                $create->select('type')
